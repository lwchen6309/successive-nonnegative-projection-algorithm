...
def plot_spectra_compare(trspec, v, t, 
    ...
    legend_str = ['t = %.1f s'%t[i] for i in time_index]
...
def plot_trspec(trace, spectra, trspec):

    t, ref_trace = trace
    v, ref_spectra = spectra
    
    fig, axes = plt.subplots(2,1)
    axes[0].plot(t, ref_trace)
    axes[0].set_xlabel('time / s')
    axes[0].set_title('artificial trace')
    axes[1].plot(v, ref_spectra)
    axes[1].set_xlabel('wavenumber / nm')
    axes[1].set_title('artificial spectra')
    plt.subplots_adjust(hspace=0.5)
    ...
    ax.set_title('artificial time-resolved spectra')
...
if __name__ == '__main__':
    ...
    plot_trspec((t, ref_trace), (v, ref_spectra), trspec)
